...
import 'package:tarzan/simon_tar/util/parse_pax_time.dart';
import 'package:tarzan/simon_tar/util/read_numeric.dart';
import 'package:tarzan/simon_tar/util/read_string.dart';
import 'package:tarzan/simon_tar/util/us_since_epoch.dart';
import 'package:tarzan/simon_tar/util/zeroes.dart';
...
void main() {
  group('readString', () {
    test('can read empty strings', () {
      expect(readStringUint8List(_bytes(''), 0, 0), '');
    });
    test('does not include trailing null', () {
      expect(readStringUint8List(_bytes('hello\x00'), 0, 6), 'hello');
    });
    test('does not require a trailing null', () {
      expect(readStringUint8List(_bytes('hello'), 0, 5), 'hello');
    });
  });
  group('readStringOrNullIfEmpty', () {
    test('returns null if empty', () {
      expect(readStringOrNullIfEmpty(_bytes(''), 0, 0), isNull);
    });
    test('can read non-empty strings', () {
      expect(readStringOrNullIfEmpty(_bytes('hello'), 0, 5), 'hello');
    });
  });
  ...
  group('readNumeric', () {
    ...
    group('base-256', () {
      ...
      testValid('\x80\x00\x00\x00\x07\x76\xa2\x22\xeb\x8a\x72\x61', 537795476381659745);
      testValid('\xf7\x76\xa2\x22\xeb\x8a\x72\x61', -615126028225187231);
      testValid('\xff\xff\xff\xff\xf7\x76\xa2\x22\xeb\x8a\x72\x61', -615126028225187231);
...
Uint8List _bytes(String str) => Uint8List.fromList(utf8.encode(str));
